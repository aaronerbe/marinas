...
//set global var to store the windyAPI object so I can check it's state later
let windyAPI;
...
function renderMap(lat, lon, marinas) {
    ...
    // Create a marker element at search location
    //need to add logic to add many markers.  
    const marker = L.marker([lat, lon]).addTo(map);

    // Add a popup to the marker (you can customize this)
    //need to pass City name into function so it can be placed here
    marker.bindPopup('<b>Your Marker</b>').openPopup();
    // Center the map on the marker
    map.setView([lat, lon], map.getZoom());

    //place markers for each marina location:
    Object.values(marinas.data).forEach(marina => {
        const { name, location, kind } = marina;
        ...
        if (kind === 'marina'){
            const marker = L.marker([lat, lon]).addTo(map);
            // Add a popup to the marker with the marina name
...
function updateMap_OLD(options){
    if (!windyAPI){
        conwole.error('Windy API not initialized')
        return
    }
    const { map } = windyAPI;
